...
`Tab` — одна вкладка. `Tabs` — контейнер для вкладок. `VERTICAL` — вкладки расположены вертикально (одна под другой, сверху вниз), а не горизонтально (как обычные браузерные вкладки).
